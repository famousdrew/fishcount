  ...
  return (
    <div className="px-3 py-3">
      ...
      <div className="flex items-center gap-3 mb-1">
        <span className={`${config.bg} text-white font-bold text-lg px-3 py-0.5 rounded`}>
          {score.status === 'insufficient' ? '?' : config.label}
        </span>
        {score.status !== 'insufficient' && (
          <span className="text-slate-300 font-bold text-lg tabular-nums">{score.total.toFixed(1)}</span>
        )}
        <span className="text-slate-100 font-medium text-sm truncate">{headlineStat}</span>
      </div>

      {/* Context line */}
      <div className="flex items-center justify-between">
        <p className="text-slate-400 text-xs truncate">{contextLine}</p>
        <p className="text-slate-600 text-xs whitespace-nowrap ml-2">{updatedTime}</p>
      </div>
    </div>
  );
}
