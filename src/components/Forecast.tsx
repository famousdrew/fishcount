        ...
        {selected.weather ? (
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              <span className="text-2xl">{getWeatherEmoji(selected.weather.conditions)}</span>
              <div>
                <span className="text-slate-100 text-xl font-bold">{selected.weather.tempF}°F</span>
                <span className="text-slate-400 text-sm ml-2">{selected.weather.conditions}</span>
              </div>
            </div>
          ...
          <div className="text-slate-500 text-sm mb-3">Weather data unavailable</div>
        )}
...
function TideChip({ event }: { event: TideEvent }) {
  ...
  return (
    <div className={`flex items-center gap-1.5 text-sm ${isHigh ? 'text-sky-400' : 'text-slate-400'}`}>
      <span className="font-medium">{isHigh ? 'H' : 'L'}</span>
      <span className="text-slate-300">{formatTideTime(event.time)}</span>
      <span className="text-slate-500 text-xs">{event.level.toFixed(1)}ft</span>
    </div>
  );
}
